...
public:
    ...
    virtual std::list< std::shared_ptr< term< T > > > children( )=0;
...
public:
    ...
    // No children, so return empty list
    std::list< std::shared_ptr< term< T > > > children( ){return std::list< std::shared_ptr< term< T > > >();}
...
public:
    function(std::string __name, uint32_t __arity, std::list<std::shared_ptr< term<T>>> __subterms );
    std::string& name(){return _name;}
    const std::string& name()const{ return _name; }
    std::list< std::shared_ptr< term< T > > > children( ){return _subterms;}
    std::ostream& pp(std::ostream&) const;
private:
    std::string _name;
    uint32_t _arity;
    std::list< std::shared_ptr< term<T> > > _subterms;
};
*/
...
/*
template<typename T>
function<T>::function(std::string __name, uint32_t __arity, std::list<std::shared_ptr< term<T>>> __subterms ):
    _name{__name},
    _arity{__arity},
